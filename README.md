# Pense Rápido! 🎮

Jogo educacional desenvolvido em Godot 4.x para crianças de 6 a 9 anos, focado em matemática e português.

## 📋 Descrição

**Pense Rápido!** é inspirado no popular jogo "漢字でGO!" e desafiaalunos a responder perguntas de matemática e gramática portuguesa antes que cresçam demais na tela! Com sistema de vidas, progressão por fases, e dificuldades ajustadas por idade, o jogo torna o aprendizado divertido e engajante.

## ✨ Características

- **Dois Modos de Jogo**: Matemática e Português
- **Três Níveis de Dificuldade**: Fácil (6-7 anos), Médio (7-8 anos), Difícil (9+ anos)
- **Sistema de Progressão**: 7 fases por jogo com aumento gradual de dificuldade
- **Sistema de Vidas**: 3 vidas por jogo, incentivando atenção e aprendizado
- **Explicações Educativas**: Feedback imediato com explicações quando errar
- **Sistema de Pontuação**: Pontos baseados na velocidade de resposta
- **Salvamento Automático**: High scores salvos por modo e dificuldade
- **Controles Totalmente por Teclado**: Acessível e fácil de usar

## 🎯 Conteúdo Educacional

### Matemática
- **Fácil**: Adição e subtração de 1 dígito (2+2, 5-3)
- **Médio**: Multiplicação e divisão básica (7×3, 18÷2)
- **Difícil**: Operações com números maiores (12×8, 144÷12)

### Português
- **Fácil**: Classificação básica de palavras (substantivo, verbo, adjetivo)
- **Médio**: Sílaba tônica, ortografia (ç, ss, sc)
- **Difícil**: Classificação prosódica (proparoxítona, paroxítona, oxítona)

## 🎮 Como Jogar

### Controles
- **Setas ← →**: Navegar entre opções
- **SPACE**: Confirmar seleção / Ativar input / Continuar
- **Seta ↓**: Alternar modo (Matemática ↔ Português)
- **ENTER**: Confirmar resposta
- **Shift**: Voltar / Desistir

### Mecânica
1. Escolha modo (Matemática ou Português) e dificuldade (Fácil, Médio, Difícil)
2. Perguntas aparecem pequenas e crescem gradualmente
3. Digite a resposta antes que fique muito grande!
4. Responda corretamente para ganhar pontos e avançar
5. Complete 7 fases sem perder todas as vidas para vencer!